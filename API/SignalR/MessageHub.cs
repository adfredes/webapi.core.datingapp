...
namespace API.SignalR
{
    public class MessageHub : Hub
    {
        ...
        public async Task SendMessage(CreateMessageDto createMessageDto)
        {
            ...
            if (group.Conections.Any(x => x.Username == recipient.UserName))
            {
                message.DateRead = DateTime.UtcNow;
            }
            else
            {
                var connections = await _tracker.GetConnerctionsForUser(recipient.UserName);
                if (connections != null)
                {
        ...
        private async Task<Group> RemoveFromMessageGroup()
        {
            ...
            var connection = group.Conections.FirstOrDefault(x => x.ConnectionId == Context.ConnectionId);
